...
import {
  Plus, Trash2, Edit2, BookOpen, Users,
  ListPlus, X, Sparkles, Check, ChevronDown, ChevronUp
} from 'lucide-react';
...
const AdminPanel = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 text-white">
      ...
      {/* STUDENTS TAB — with enrollment counts and progress meters */}
      {activeTab === 'students' && (
        <div className="space-y-4">
          ...
          {students.map(student => {
            ...
            const completedCount = student.enrollments?.filter(e => e.progress === 100).length || 0;

            return (
              <div key={student._id} className="glass-panel rounded-2xl overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="text-center hidden sm:block">
                      <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider">Enrolled</p>
                    ...
                    {enrollmentCount > 0 && (
                      <div className="text-center hidden md:block">
                        <p className="text-xs text-slate-500 font-semibold uppercase tracking-wider mb-1">Avg Progress</p>
                        <div className="w-24 h-2 bg-slate-800 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-brand-500 to-indigo-500 rounded-full transition-all"
                            style={{
                              width: `${Math.round(
                                student.enrollments.reduce((sum, e) => sum + e.progress, 0) / enrollmentCount
                              )}%`
                            }}
                          ></div>
                        </div>
                ...
                {/* Expanded: Course Progress Details */}
                {isExpanded && (
                      ...
                      <>
                        ...
                        {student.enrollments.map(enrollment => (
                          <div key={enrollment.enrollmentId}
                            ...
                            <div className="flex items-center justify-between text-xs text-slate-500">
                              <span>
                                {enrollment.completedLessonsCount} / {enrollment.totalLessonsCount} lessons done
                              </span>
